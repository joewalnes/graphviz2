//
// Time-stamp: <30.10.2003 14:33:31h liekweg>
//
// This library is distributed under the BSD license.
// See license.txt for more information.
//
// $Id: DotColorAttr.java,v 1.3 2009/06/03 01:10:50 ellson Exp $
//

package dot;

/**
   <P>Quick and dirty hack to MAKE COLORS FAST.</P>

   @since   Sun Oct 26 22:08:55 2003
   @author  Florian Liekweg <TT>&lt;liekweg@ipd.info.uni-karlsruhe.de&gt;</TT>, Universität Karlsruhe (TH), Germany
 */

public class DotColorAttr extends DotStringAttr
{
  /**
     <P></P>
  */
  public DotColorAttr (String label)
  {
    super ("color", label);
  }
}
